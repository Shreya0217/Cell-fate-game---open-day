...
def plot_grid(grid):
    cmap = ListedColormap(["black", "green", "blue", "purple"])
    fig, ax = plt.subplots(figsize=(3,3), dpi=80)  # smaller figure
...
with left_col:
    ...
    unique, counts = np.unique(st.session_state.grid, return_counts=True)
    stats = dict(zip(unique, counts))
    st.markdown(
        f"🟢 Differentiated: **{stats.get(DIFF,0)}**  \n"
        f"🔵 Stem Cells: **{stats.get(SELF,0)}**  \n"
        f"⬛ Empty: **{stats.get(EMPTY,0)}**  \n"
        f"🟣 Cancer: **{stats.get(CANCER,0)}**"
    )
    
    # Buttons centered
    st.markdown("---")
    st.subheader("Simulation Controls & Cancer Injection")
    btn_col1, btn_col2, btn_col3 = st.columns(3)
    
    with btn_col1:
        if st.button("▶ Next 10 Steps"): run_steps(10)
        if st.button("🟣 Add 10"): add_cancer_cells(10)
    with btn_col2:
        if st.button("▶ Next 20 Steps"): run_steps(20)
        if st.button("🟣 Add 20"): add_cancer_cells(20)
    with btn_col3:
        if st.button("▶ Next 50 Steps"): run_steps(50)
        if st.button("🟣 Add 50"): add_cancer_cells(50)
